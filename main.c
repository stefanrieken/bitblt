...
void write_intro_text(planar_image * on) {
  ...
  draw_text(on, "Next up ====> a moving demo of planar bitblt,", 2, false);
  draw_text(on, "which is somewhat more refined.", 3, false);
}
...
void * demo(void * args) {
  ...
  write_intro_text(display);

  packed_image * packed_bg = to_packed_image(pack(display), background->size.x, background->size.y, background->depth);
